...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    def gather_training_rays():
        if data_dict['irregular_shape']:
            rgb_tr_ori = [images[i].to(
                'cpu' if cfg.data.load2gpu_on_the_fly else device) for i in i_train]
        else:
        ...
        def batch_index_sampler(): return next(index_generator)
    ...
    for global_step in trange(1 + start, 1 + cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            n_rest_scales = len(cfg_train.pg_scale) - \
                cfg_train.pg_scale.index(global_step) - 1
        ...
        loss = cfg_train.weight_main * \
            F.mse_loss(render_result['rgb_marched'], target)
        psnr = utils.mse2psnr(loss.detach())
        if cfg_train.weight_entropy_last > 0:
            pout = render_result['alphainv_last'].clamp(1e-6, 1 - 1e-6)
            entropy_last_loss = -(pout * torch.log(pout) +
                                  (1 - pout) * torch.log(1 - pout)).mean()
        ...
        if cfg_train.weight_rgbper > 0:
            rgbper = (render_result['raw_rgb'] -
                      target[render_result['ray_id']]).pow(2).sum(-1)
            rgbper_loss = (
                rgbper * render_result['weights'].detach()).sum() / len(rays_o)
        ...
        if global_step % args.i_weights == 0:
            path = os.path.join(cfg.basedir, cfg.expname,
                                f'{stage}_{global_step:06d}.tar')
            torch.save({
            ...
            print(
                f'scene_rep_reconstruction ({stage}): saved checkpoints at', path)
...
def train(args, cfg, data_dict):
    ...
    if cfg.coarse_train.N_iters > 0:
        ...
        coarse_ckpt_path = os.path.join(
            cfg.basedir, cfg.expname, f'coarse_last.tar')
    else:
...
if __name__ == '__main__':
    ...
    if args.export_bbox_and_cams_only:
        ...
        for c2w, (H, W), K in zip(poses[i_train], HW[i_train], Ks[i_train]):
            ...
            cam_lst.append(
                np.array([cam_o, *(cam_o + cam_d * max(near, far * 0.05))]))
    ...
    if args.export_coarse_only:
        ...
        with torch.no_grad():
            ckpt_path = os.path.join(
                cfg.basedir, cfg.expname, 'coarse_last.tar')
            model = utils.load_model(dvgo.DirectVoxGO, ckpt_path).to(device)
            alpha = model.activate_density(
                model.density.get_dense_grid()).squeeze().cpu().numpy()
            rgb = torch.sigmoid(model.k0.get_dense_grid()).squeeze().permute(
                1, 2, 3, 0).cpu().numpy()
    ...
    if args.render_train:
        ...
        rgbs, depths, bgmaps = render_viewpoints(
            ...
            gt_imgs=[data_dict['images'][i].cpu().numpy()
                     for i in data_dict['i_train']],
            savedir=testsavedir, dump_images=args.dump_images,
            eval_ssim=args.eval_ssim, eval_lpips_alex=args.eval_lpips_alex, eval_lpips_vgg=args.eval_lpips_vgg,
            **render_viewpoints_kwargs)
        imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'),
                         utils.to8b(rgbs), fps=30, quality=8)
        imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(
            1 - depths / np.max(depths)), fps=30, quality=8)

    # render testset and eval
    if args.render_test:
        testsavedir = os.path.join(
            cfg.basedir, cfg.expname, f'render_test_{ckpt_name}')
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        rgbs, depths, bgmaps = render_viewpoints(
    ...
    if args.render_video:
        ...
        depth_vis = plt.get_cmap('rainbow')(
            1 - np.clip((depths_vis - dmin) / (dmax - dmin), 0, 1)).squeeze()[..., :3]
        imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'),
                         utils.to8b(depth_vis), fps=30, quality=8)
