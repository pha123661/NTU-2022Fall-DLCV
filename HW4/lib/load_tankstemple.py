...
def load_tankstemple_data(basedir, movie_render_kwargs={}):
    ...
    radcircle = movie_render_kwargs.get(
        'scale_r', 1.0) * np.linalg.norm(poses[:, :3, 3] - centroid, axis=-1).mean()
    ...
    render_poses = np.concatenate([render_poses, np.broadcast_to(
        poses[0, :3, -1:], render_poses[:, :3, -1:].shape)], -1)
