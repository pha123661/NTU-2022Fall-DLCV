...
def focal_loss(logits, labels, alpha=None, gamma=2):
    ...
    """
    assert torch.sum(labels) > 0, "All labels are zero in Focal Loss"
...
class ClassBalancedLoss(torch.nn.Module):
    ...
    def forward(
        ...
        # ignore
        not_ignore_mask = (labels != self.ignore_idx)  # a boolean tensor
        logits, labels = logits[not_ignore_mask], labels[not_ignore_mask]
        ...
        if self.class_balanced:
            assert len(
                self.samples_per_class) == num_classes, f"sample size of {len(self.samples_per_class)} does not match input logits shape {logits.shape} with {num_classes} classes"
        ...
        if self.loss_type == "focal_loss":
            cb_loss = focal_loss(logits, labels_one_hot,
                                 alpha=weights, gamma=self.fl_gamma)
