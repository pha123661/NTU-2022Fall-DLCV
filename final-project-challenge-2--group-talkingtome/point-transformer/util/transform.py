...
# class RandomSubSampling(object):
#     def __init__(self, p=0.2):
#         self.p = p

#     def __call__(self, coord, feat, label):
#         l = len(coord)
#         sub_sampling = [1]*np.floor(l*self.p).astype(int) + [0]*np.ceil(l*(1-self.p)).astype(int)
#         np.random.shuffle(sub_sampling)
#         return list(np.array(coord)[sub_sampling]), list(np.array(feat)[sub_sampling]), list(np.array(label)[sub_sampling])