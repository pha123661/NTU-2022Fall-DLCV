...
def collate_fn(batch):
    ...
    # # !! shuffle will makes performance worce
    # shuf_idx = np.arange(count)
    # np.random.shuffle(shuf_idx)
    # return torch.cat(coord)[shuf_idx], torch.cat(feat)[shuf_idx], torch.cat(label)[shuf_idx], torch.IntTensor(offset)
    return torch.cat(coord), torch.cat(feat), torch.cat(label), torch.IntTensor(offset)


def data_prepare(coord, feat, label, split='train', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False, mix3d=False):
    if transform:
    ...
    if voxel_max and label.shape[0] > voxel_max:
        init_idx = np.random.randint(
            label.shape[0]) if 'train' in split else label.shape[0] // 2
        crop_idx = np.argsort(
